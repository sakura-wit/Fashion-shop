...
export function AoPage() {
    ...
    async function getDataAo() {
        setIsLoading(true)
        const res = await ProductService.getProductApi.getProductFilterAll('name', 'ao')
        if (res.data) {
    ...
    async function getDataPanigation(page) {
        setIsLoading(true)
        const res = await ProductService.getProductApi.getDataPanigation(page, 'Áo')
        if (res.data) {
            dispash(update(res.data))
        }
        setIsLoading(false)
